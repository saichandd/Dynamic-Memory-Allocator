...
Test(sf_memsuite_student, malloc_check_epilogue_prevalloc, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	int *x = sf_malloc(2008);
	int *y = sf_malloc(2000);
...
Test(sf_memsuite_student, coalasce_free, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	int *x = sf_malloc(2008);
	int *y = sf_malloc(2000);
...
Test(sf_memsuite_student, malloc_crazy, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	int *x = sf_malloc(2008);
	int *y = sf_malloc(2000);
	int *a = sf_malloc(1);
	...
	*d = 10;
	...
	cr_assert(sf_errno == 0, "sf_errno is not zero!");
	cr_assert(sf_mem_start() + 2*PAGE_SZ == sf_mem_end(), "Allocated more than necessary!");
}



Test(sf_memsuite_student, free_crazy, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	int *x = sf_malloc(2008);
	int *y = sf_malloc(2000);
	int *a = sf_malloc(1);
